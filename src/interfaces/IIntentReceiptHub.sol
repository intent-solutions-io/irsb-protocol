...
interface IIntentReceiptHub {
    ...
    event SettlementProofSubmitted(
        bytes32 indexed receiptId,
        bytes32 proofHash
    );
    ...
    function getReceipt(
        bytes32 receiptId
    ) external view returns (Types.IntentReceipt memory receipt, Types.ReceiptStatus status);
    ...
    function getReceiptsBySolver(
        ...
        uint256 offset,
        uint256 limit
    ) external view returns (bytes32[] memory receiptIds);

    /// @notice Get receipts by intent hash
    /// @param intentHash Intent to query
    /// @return receiptIds Array of receipt IDs
    function getReceiptsByIntent(
        bytes32 intentHash
    ) external view returns (bytes32[] memory receiptIds);
